...
function mostrarpergunta () {
     if(atual > = pergunta.legth){
        ...
        function mostrarAlternativas () {
            for(constant alternativa of perguntaAtual.alternativa){
                const boataoAlternativa = document.createElement("button");
                boataoAlternativa.textContent = alternativa.texto;
                boataoAlternativa.addEventListener("click", () =>
                    respostaSelecionada(alternativa)) ;
                caixaAlternativas.appendChild(boataoAlternativa);
                 }
               }
               function respostaSelecionada (opcaoSelecionada){
                const afirmacao = opcaoSelecionada.afirmacao;
                históriafinal+= afirmacao + "  ";
                mostrarpergunta () ;
              } 
              function mostrarResultado () {
                caixaPerguntas.textContent ="Resumindo...";
                textoResultado.textContent = históriafinal;
                caixaAlternativas.textContent = "";
             }
             mostrarpergunta ();
